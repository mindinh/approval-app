...
export function buildPoZmakItemsTable(rawItems?: any[], parentCurrency?: string): DetailTableModel | null {
    ...
    const rows: DetailTableRow[] = rawItems.map((item, idx) => {
        ...
        return {
            ...
            values: {
                ...
                internalOrder: formatCodeWithText(item.internalOrder || item.orderInternalId || item.OrderInternalID, item.orderInternalName || item.OrderInternalName || item.orderInternalIdName || item.internalOrderName || item.internalOrderText),
    ...
    return {
        ...
        columns: [
            ...
            { key: 'internalOrder', label: 'Internal Order' },
