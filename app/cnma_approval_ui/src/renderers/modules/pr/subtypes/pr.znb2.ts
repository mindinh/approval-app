...
export function buildPrZnb2ItemsTable(rawItems?: any[], parentCurrency?: string): DetailTableModel | null {
    if (!rawItems || rawItems.length === 0) return null;

    const hasGlAccount = rawItems.some((item) => {
        const code = item.glAccount != null ? String(item.glAccount).trim() : '';
        const text = (item.glAccountName || item.glAccountText) != null ? String(item.glAccountName || item.glAccountText).trim() : '';
        return code !== '' || text !== '';
    });
    ...
    const columns = [
        ...
        ...(hasGlAccount ? [{ key: 'glAccount', label: 'G/L Account' }] : []),
        { key: 'commitmentItem', label: 'Commitment Item' }
    ];

    return {
        id: 'pr-znb2-items',
        title: 'Line Items (Non-Trade PR)',
        columns,
        rows,
