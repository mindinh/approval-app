...
export function buildPrZmakItemsTable(rawItems?: any[], parentCurrency?: string): DetailTableModel | null {
    ...
    const rows: DetailTableRow[] = rawItems.map((item, idx) => {
        ...
        return {
            ...
            values: {
                ...
                internalOrder: formatCodeWithText(item.orderInternalId || item.internalOrder || item.OrderInternalID, item.orderInternalName || item.OrderInternalName || item.orderInternalIdName || item.internalOrderName || item.internalOrderText),
