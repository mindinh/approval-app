...
import { Button, Textarea, Label, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@cnma/react-ui';
...
export function DecisionPanel({ decisions, onExecute, isExecuting, isMobile }: DecisionPanelProps) {
    ...
    return (
        <>
            ...
            <div className={cn("flex flex-wrap justify-end gap-2 text-sm", isMobile ? "w-full flex-row" : "")}>
                {[...decisions]
                    .sort((a, b) => {
...
function DecisionButton({
    ...
    if (isMobile) {
        if (nature === 'NEGATIVE') {
            mobileClasses = "bg-primary hover:bg-primary-hover text-white flex-1 rounded-xl h-10 shadow-sm border-0";
        } else if (nature === 'POSITIVE') {
            mobileClasses = "bg-success hover:bg-success-hover text-white flex-1 rounded-xl h-10 shadow-sm border-0";
        } else {
    ...
    return (
        <Button
            ...
            onClick={onClick}
            disabled={disabled}
            className={cn("min-w-24", mobileClasses)}
        >
            {isExecuting ? (
                <Loader2 className="size-4 animate-spin" />
            ) : !isMobile && (
