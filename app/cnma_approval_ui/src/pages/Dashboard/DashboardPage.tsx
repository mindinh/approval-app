    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
            ...
            <text x={cx} y={cy + 20} textAnchor="middle" dominantBaseline="central"
                className="fill-muted-foreground text-xs font-semibold tracking-wider">
                {centerLabel}
            </text>
        </svg>
    );
...
export default function DashboardPage() {
    ...
    const dashboardContent = (
        <div className="flex flex-col min-h-screen bg-background">
            ...
            {isMobile && (
                <div
                    className="px-4 py-3 flex items-center shadow-sm relative z-20 shrink-0 w-full min-h-16 bg-gradient-to-r from-primary to-primary-hover"
                >
                    <Button
                        variant="ghost"
                        size="icon"
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <Button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-colors bg-background border border-border text-primary hover:text-primary-hover"
                        ...
                        {hasFilters && (
                            <Button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-colors bg-error-bg text-destructive border border-destructive hover:bg-destructive/10"
                            >
                                <X size={12} />
                ...
                <div className="px-4 pb-8 md:px-8 space-y-5">
                    ...
                    {hasFilters && (
                        <div className="flex items-center gap-2 flex-wrap min-h-8">
                            ...
                            {selectedType && (
                                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-warning-bg text-warning border border-warning">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-6">
                        ...
                        <motion.div
                            ...
                            className="p-5 md:p-6 relative rounded-2xl border border-border bg-card shadow-sm"
                        >
                            {isRefetching && <RefetchOverlay />}
                            <div className="flex flex-col items-center">
                                <h3 className="text-xs md:text-sm font-bold mb-5 tracking-widest text-foreground">
                                ...
                                />
                                {/* Legend */}
                                <div className="mt-6 w-full max-w-64 space-y-2.5">
                                    {donutSegments.map((seg) => {
                                        ...
                                        return (
                                            <Button
                                                ...
                                                className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg transition-all duration-200 h-auto"
                                                style={{
                        ...
                        >
                            ...
                            <div className="px-5 pt-5 pb-2 md:px-6 md:pt-6">
                                <h3 className="text-xs md:text-sm font-bold tracking-widest text-foreground">
                            ...
                            <div className="px-5 pb-4 md:px-6 md:pb-5 flex items-center gap-4 md:gap-6 border-t border-border">
                                {STATUS_LABELS.map((status) => {
                                    ...
                                    return (
                                        <Button
                                            ...
                                            className="flex items-center gap-1.5 md:gap-2 pt-3 transition-opacity h-auto p-0 hover:bg-transparent"
                                            style={{ opacity: isFiltered ? 0.35 : 1 }}
                                        >
                                            <span
                                                className={cn(
                                                    "w-3 h-3 md:w-3.5 md:h-3.5 rounded-sm shrink-0",
                                                    isActive && "outline outline-2 outline-offset-2"
                                                )}
                                                style={{
                                                    backgroundColor: STATUS_COLORS[status],
                                                    outlineColor: isActive ? STATUS_COLORS[status] : undefined,
                                                }}
                                            />
                                            <span
                    ...
                    >
                        ...
                        <div
                            className="overflow-auto rounded-b-2xl max-h-96"
                        >
                            {tableRows.length === 0 ? (
                                ...
                                <Table>
                                    <TableHeader className="sticky top-0 z-10 bg-muted">
                                        <TableRow className="hover:bg-muted border-b">
                                            <TableHead className="px-5 py-3 text-xs font-bold tracking-wider text-muted-foreground w-16">
                                                {t('dashboard.table.rowNumber')}
                                            </TableHead>
                                            <TableHead className="px-5 py-3 text-xs font-bold tracking-wider text-muted-foreground">
                                                {t('dashboard.table.docNumber')}
                                            </TableHead>
                                            <TableHead className="px-5 py-3 text-xs font-bold tracking-wider text-muted-foreground">
                                                {t('dashboard.table.status')}
                                            </TableHead>
                                            <TableHead className="px-5 py-3 text-xs font-bold tracking-wider text-muted-foreground text-right">
    ...
    // ── Layout: global sidebar layout wraps this page ──
    if (isMobile) {
        return (
