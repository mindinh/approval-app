...
async function main() {
  ...
  if (directJwt) {
    ...
    // Also save to root .env for backend local hybrid testing
    const rootEnvPath = path.resolve(projectRoot, '../../.env');
    await upsertEnvVars(rootEnvPath, {
      SAP_DEV_JWT: directJwt.trim()
    });
    
    console.log(`[pp-token] Saved provided JWT to ${envPath} and ${rootEnvPath}`);
  ...
  const rootEnvPath = path.resolve(projectRoot, '../../.env');
  await upsertEnvVars(rootEnvPath, {
    SAP_DEV_JWT: accessToken
  });

  console.log(`[pp-token] Saved VITE_PP_JWT to ${envPath} and SAP_DEV_JWT to ${rootEnvPath}`);
  if (expiresAt) {
